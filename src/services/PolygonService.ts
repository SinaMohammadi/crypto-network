...
export class PolygonService extends BaseBlockchainService {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            if (process.env.POLYGON_PRIVATE_KEY || process.env.PRIVATE_KEY) {
                const privateKey = process.env.POLYGON_PRIVATE_KEY || process.env.PRIVATE_KEY;
                this.wallet = new ethers.Wallet(privateKey!, this.provider);
                this.logger.info(`Polygon wallet initialized: ${this.wallet.address}`);
            }

            this.logger.info(`Polygon service initialized successfully on ${network.name} (Chain ID: ${network.chainId})`);
        } catch (error) {
    ...
    async getBalance(address: string): Promise<number> {
        try {
            ...
            this.logger.debug(`Balance for ${address}: ${balanceMatic} MATIC`);
            return balanceMatic;
        } catch (error) {
    ...
    async sendTransaction(request: TransactionRequest): Promise<TransactionResponse> {
        try {
            if (!this.wallet) {
                throw new Error('Wallet not initialized. Please provide POLYGON_PRIVATE_KEY or PRIVATE_KEY');
            }
    ...
    async createWallet(options?: CreateWalletOptions): Promise<WalletCreationResponse> {
        try {
            ...
            const targetMnemonic = options?.mnemonic || process.env.POLYGON_SEED;
            ...
            if (targetMnemonic) {
                ...
                if (options?.index !== undefined) {
                    ...
                    this.logger.info(`Created Polygon wallet with derivation path: ${usedDerivationPath}`);
                } else {
                ...
                if (options?.index !== undefined) {
                    ...
                    if (mnemonic) {
                        ...
                        this.logger.info(`Created new Polygon wallet with derivation path: ${usedDerivationPath}`);
                    } else {
            ...
            this.logger.info(`Created new Polygon wallet: ${wallet.address}`);
